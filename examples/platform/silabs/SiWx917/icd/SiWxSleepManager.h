...
/**
 * @brief SiWxSleepManager is a singleton class that manages the sleep modes of the SiWx917 device.
 *        The class contains the buisness logic associated with optimizing the sleep states of the SiWx Platform based on the Matter
 *        SDK internal states
 *
 ...
 *        For LIT ICDs, the logic is based on the ICDManager operating modes.
 */
class SiWxSleepManager : public chip::app::ICDStateObserver, public chip::app::ReadHandler::ApplicationCallback
{
public:
    SiWxSleepManager(const SiWxSleepManager &)             = delete;
    SiWxSleepManager & operator=(const SiWxSleepManager &) = delete;

    static SiWxSleepManager & GetInstance() { return mInstance; }

    /**
     * @brief Init function that configure the SiWxSleepManager APIs based on the type of ICD
...
private:
    SiWxSleepManager()  = default;
    ~SiWxSleepManager() = default;

    static SiWxSleepManager mInstance;
};
